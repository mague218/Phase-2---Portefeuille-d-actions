...
class Portefeuille:
    ...
    def valeur_totale(self, date=None):
        ...
        valeur_actions = sum(
            self.bourse.obtenir_prix_historique(sym, date.strftime('%Y-%m-%d')) * quant
    ...
    def valeur_actions(self, symboles, date=None):
        ...
        valeur_actions = sum(
            self.bourse.obtenir_prix_historique(sym, date.strftime("%Y-%m-%d")) * quant
            for sym, quant in self.actions.items() if sym in symboles)
    ...
    def  valeur_projetee(self, date, taux_rendement):
        ...
        for sym, quant in self.actions.items():
            taux_rendement_titre = (
                taux_rendement
                if isinstance(taux_rendement, (int, float))
                else taux_rendement.get(sym, 0)
            )
